...
import { MapPin, Phone, Mail, ArrowRight, Send, User, MessageSquare, Building2, Globe2, Users, Award, Rocket, LayoutGrid, Clock, ShieldCheck, Network, Wrench, Briefcase, BadgeCheck, Handshake } from "lucide-react";
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          <div className="grid lg:grid-cols-[1fr_1fr] gap-16 lg:gap-8 items-center">
            ...
            <motion.div variants={containerVariants} initial="hidden" animate={heroInView ? "visible" : "hidden"} className="max-w-2xl lg:pr-8">
              <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-50/80 rounded-full mb-8 border border-blue-100 max-w-full">
                <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse shadow-[0_0_8px_rgba(15,76,255,0.6)] shrink-0" />
                <span className="text-[9px] sm:text-xs font-bold tracking-[0.1em] sm:tracking-[0.15em] text-primary uppercase truncate sm:whitespace-normal">LEADING TECHNOLOGY SOLUTIONS PROVIDER</span>
              </motion.div>
      ...
      <section ref={formRef} className="py-20 bg-[#f8fafc]">
        <div className="container mx-auto px-6 lg:px-12">
          ...
          >
            {/* Form */}
            <motion.form variants={itemVariants} onSubmit={handleSubmit} className="space-y-6 bg-white rounded-2xl border-2 border-blue-100 shadow-lg p-5 sm:p-8">
        ...
        <div className="container mx-auto px-6 lg:px-12">
          <div className="text-center max-w-3xl mx-auto mb-10">
            ...
            <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">
              Find Us <span className="text-primary italic font-light">Here</span>
            </h2>
            <div className="w-10 h-0.5 bg-primary mx-auto rounded-full"></div>
          </div>
      ...
      {/* FAQ */}
      <section ref={faqRef} className="py-24 lg:py-32 bg-[#f8fbff]">
        <div className="container mx-auto px-6 lg:px-12 max-w-7xl">
          <motion.div
            variants={containerVariants}
            initial="hidden"
            animate={faqInView ? "visible" : "hidden"}
            className="flex flex-col gap-10 sm:gap-16 w-full max-w-full overflow-hidden"
          >
            <div className="text-center max-w-3xl mx-auto mb-10">
              <motion.span
                variants={itemVariants}
                className="inline-block text-xs font-bold tracking-[0.15em] uppercase text-primary mb-3 text-center"
              >
                FREQUENTLY ASKED QUESTIONS
              </motion.span>
              <motion.h2
                variants={itemVariants}
                className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4 text-center"
              >
                Got Questions? <span className="text-primary italic font-light">We've Got Answers</span>
              </motion.h2>
              <motion.p variants={itemVariants} className="text-muted-foreground text-[15px] sm:text-[16px] max-w-2xl mx-auto mb-4">
                Get answers to common questions about our services and solutions.
              </motion.p>
              <motion.div variants={itemVariants} className="w-10 h-0.5 bg-primary mx-auto rounded-full" />
            </div>

            <div className="grid lg:grid-cols-[1fr_2.5fr] gap-8 items-start">
              {/* Left Side: Need more clarity Card */}
              <motion.div variants={itemVariants} className="bg-gradient-to-br from-[#eef4ff] to-[#f8fbff] rounded-3xl p-8 border border-blue-50 text-center shadow-sm relative overflow-hidden flex flex-col h-full items-center justify-center min-h-[400px]">
                ...
                <svg className="absolute bottom-0 right-0 w-full h-1/2 opacity-20 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                  <path d="M0,100 C30,80 70,80 100,100" fill="none" stroke="#004E98" strokeWidth="2" />
                ...
                <a href="#contact-form" className="w-full inline-flex items-center justify-center gap-2 bg-[#002B7F] text-white py-3.5 px-6 rounded-xl font-semibold hover:bg-[#001f5c] transition-colors relative z-10">
                  Contact Us <ArrowRight size={16} />
                </a>
              </motion.div>

              <motion.div variants={itemVariants} className="space-y-4">
                <Accordion type="single" collapsible className="space-y-3 sm:space-y-4 w-full">
                  {faqs.map((faq, i) => {
                    const icons = [LayoutGrid, Clock, User, ShieldCheck, Network, Wrench, Briefcase];
                    const Icon = icons[i % icons.length];
                    return (
                      <AccordionItem key={i} value={`faq-${i}`} className="bg-white rounded-xl border-none shadow-[0_4px_20px_rgb(0,0,0,0.03)] px-1 sm:px-2 overflow-hidden">
                        <AccordionTrigger className="text-left font-semibold text-[#0b162c] text-[14px] sm:text-base hover:no-underline py-4 px-3 sm:py-5 sm:px-4 flex items-center gap-3 sm:gap-4 whitespace-normal">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#1853a0] flex items-center justify-center shrink-0">
                            <Icon className="text-white w-4 h-4 sm:w-[18px] sm:h-[18px]" />
                          </div>
                          <span className="flex-1 text-left break-words leading-snug">{faq.question}</span>
                        </AccordionTrigger>
                        <AccordionContent className="text-gray-600 leading-relaxed px-3 pb-4 pl-[56px] sm:px-4 sm:pb-5 sm:pl-[72px] text-[13.5px] sm:text-base">
                          {faq.answer}
                        </AccordionContent>
                      </AccordionItem>
                    );
            ...
            {/* Bottom Features */}
            <motion.div variants={itemVariants} className="bg-[#f4f8fe] rounded-3xl p-6 sm:p-10 border border-blue-50">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  { icon: ShieldCheck, title: "Confidential", desc: "Your information is safe with us." },
                  { icon: Clock, title: "Quick Response", desc: "We respond within 24 business hours." },
                  { icon: BadgeCheck, title: "Expert Guidance", desc: "Get advice from industry experts." },
                  { icon: Handshake, title: "Client First", desc: "Your success is our top priority." },
                ].map((feature, idx) => (
                  <div key={idx} className={`flex items-start gap-4 ${idx !== 0 ? 'lg:border-l lg:border-blue-100 lg:pl-8' : ''}`}>
                    <div className="w-12 h-12 rounded-full bg-[#3d72d7] flex items-center justify-center shrink-0 shadow-md">
                      <feature.icon size={22} className="text-white" />
                    </div>
                    <div>
                      <h4 className="font-bold text-[#002B7F] text-sm mb-1">{feature.title}</h4>
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-8 py-4 text-sm font-semibold bg-white text-[#002B7F] rounded-xl hover:bg-blue-50 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
              >
                Contact Us Today
                <ArrowRight size={16} />
              </a>
